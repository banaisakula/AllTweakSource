#include "LOURootListController.h"

@implementation LOURootListController

- (void)viewDidLoad {

    [super viewDidLoad];

    self.appearanceSettings = [LOUAppearanceSettings new];
    self.hb_appearanceSettings = [self appearanceSettings];


    self.preferences = [[HBPreferences alloc] initWithIdentifier:@"love.litten.loupreferences"];


    self.enableSwitch = [UISwitch new];
    [[self enableSwitch] setOnTintColor:[UIColor colorWithRed:0.28 green:0.56 blue:0.46 alpha:1]];
    ...
    [[self iconView] setImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/LouPreferences.bundle/icon.png"]];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/Lou.disabled"]) {
        [[self enableSwitch] setEnabled:NO];

        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Lou" message:@"Lou has detected that you have disabled it with iCleaner Pro, here are some quick actions you can perform" preferredStyle:UIAlertControllerStyleAlert];
        ...
        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Lou" message:@"Lou has detected that you have DayNightSwitch installed, which causes issues with Lou's preferences\n\n To continue, please disable DayNightSwitch with iCleaner Pro or uninstall it temporarily" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)resetPrompt {
    ...
    UIAlertController* resetAlert = [UIAlertController alertControllerWithTitle:@"Lou" message:@"Do you really want to reset your preferences?" preferredStyle:UIAlertControllerStyleActionSheet];
...
- (void)respring {
    ...
    [UIView animateWithDuration:1 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/shuffle.dylib"])
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Lou"]];
        else
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Tweaks&path=Lou"]];
    }];
